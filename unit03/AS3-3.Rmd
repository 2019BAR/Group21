...
prop.table(table(CPSData$Citizenship))
...
#5★★★
lapply(CPSData[c(-2,-5,-7,-12)],function(x) table(is.na(CPSData$Married),x))
...
```{r}
table(is.na(CPSData$MetroAreaCode),CPSData$State)
```
...
```{r}
tapply(is.na(CPSData$MetroAreaCode),CPSData$Region,mean) %>%sort

```

Which state has the largest proportion of non-metropolitan interviewees, ignoring states where all interviewees were non-metropolitan?
```{r}
a<-table(!is.na(CPSData$MetroAreaCode),CPSData$State)
a
...
```{r}
CPS= merge(CPSData, metro, by.x="MetroAreaCode", by.y="Code", all.x=TRUE)
#
```
...
table(CPS$MetroArea)%>%sort%>%tail(1)
...
```{r}
a<-tapply(CPS$Race=="Asian",CPS$MetroArea,mean)%>%sort
a
a[a>0.2]
#
```
...
```{r}
tapply(CPS$Education==c("No high school diploma"),CPS$MetroArea,mean,na.rm=T)%>%sort%>%head(1)
#
```
...
```{r}
CPS1<-merge(CPS,country,by.x="CountryOfBirthCode",by.y="Code",all.x=T)
#
```

How many interviewees have a missing value for the new metropolitan area variable?
```{r}
sum(is.na(CPS1$MetroArea))
#
```

__§ 4.2__ Among all interviewees born outside of North America, which country was the most common place of birth?
```{r}
table(CPS1$Country)%>%sort%>%tail()
#墨西哥(中南美)
```
...
```{r}
table(CPS1$Country=="United States",CPS1$MetroArea=="New York-Northern New Jersey-Long Island, NY-NJ-PA")%>%prop.table(1)
...
```{r}
tapply(CPS1$Country=="India",CPS1$MetroArea,sum,na.rm=T)%>%sort%>%tail(1)
#
```

In Brazil?
```{r}
tapply(CPS1$Country=="Brazil",CPS1$MetroArea,sum,na.rm=T)%>%sort%>%tail(1)
#
```

In Somalia?
```{r}
tapply(CPS1$Country=="Somalia",CPS1$MetroArea,sum,na.rm=T)%>%sort%>%tail(1)
#
```
