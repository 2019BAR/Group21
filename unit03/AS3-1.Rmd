...
```{r}
sort(table(format(ts, "%u")))  #Friday
```

【2.5】 Which month has the largest number of motor vehicle thefts for which an arrest was made?
```{r}
sort(table(format(ts[D$Arrest], '%m')))  #January
```

```{r}
max(table(D$Month[D$Arrest==TRUE]))
...
(a) 2002-2012: crime decreases
(b) 2005-2008: crime decreases
(c) 2009-2011: crime increases
...
```{r}
prop.table(table(D$Arrest, format(ts, "%Y")),2) %>%round(3)  #0.104 in2001
```

【3.4】 For what proportion of motor vehicle thefts in 2007 was an arrest made?
```{r}
prop.table(table(D$Arrest, format(ts, "%Y")),2) %>%round(3) #0.085 in 2007
```

【3.5】 For what proportion of motor vehicle thefts in 2012 was an arrest made?
```{r}
prop.table(table(D$Arrest, format(ts, "%Y")),2) %>%round(3)  #0.039 in 2012
```
...
```{r}
sum(D$LocationDescription %in% top5)  #177510 observations in Top5
```
...
```{r}
sort(table(format(ts[D$LocationDescription =="GAS STATION"], '%u')))   #Saturday
```

【4.5】 On which day of the week do the fewest motor vehicle thefts in residential driveways happen?
```{r}
sort(table(format(ts[D$LocationDescription =="DRIVEWAY - RESIDENTIAL"], '%u')))  #Saturday
```
