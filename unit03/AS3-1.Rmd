...
```{r}
date = as.POSIXct(mvt$Date, format="%m/%d/%y %H:%M")
date
```

```{r fig.height=2.5, fig.width=8}
par(cex=0.7)
hist(date,"year",las=2,freq=T,xlab="")
```
...
```{r}
table(date[mvt$Arrest]>as.POSIXct("2007-01-01"))
#他說有被抓到的人上下半年度比較，應該要指定被抓到情況
```

【3.3】 For what proportion of motor vehicle thefts in 2001 was an arrest made?
```{r}
sum(date[mvt$Arrest]<as.POSIXct("2001-12-31"))/sum(date<as.POSIXct("2001-12-31"))
table(mvt$Arrest, format(date,'%Y')) %>% prop.table(2) %>% round(3)
...
```{r}
tapply(mvt$Arrest, format(date,'%Y'), mean) %>% round(3)
```
...
```{r}
top<-table(mvt$LocationDescription)%>%sort%>%tail(6)
top
```
...
```{r}
tapply(mvt$Arrest,mvt$LocationDescription,mean)%>%
tail(1)%>% round(3)
```

【4.4】 On which day of the week do the most motor vehicle thefts at gas stations happen?
```{r}
table(format(date[mvt$LocationDescription=="GAS STATION"],"%u"))%>%sort%>%tail(1)
```

【4.5】 On which day of the week do the fewest motor vehicle thefts in residential driveways happen?
```{r}
table(format(date[mvt$LocationDescription== "DRIVEWAY - RESIDENTIAL"],"%u"))%>%sort%>%head(1)
