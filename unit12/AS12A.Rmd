...
```{r}
#blue line:線性組合(logit)=0的線，當點落在此線上，代表y=1的機率=0.5。
#cyan lines:藍線以上(右)為線性組合(logit)>0的線，當點落在此線上，代表y=1的機率>0.5，離藍線越遠的線機率越接近1。藍線以下(左)為線性組合(logit)<0的線，當點落在此線上，代表y=1的機率<0.5，離藍線越遠的線機率越接近0。
```

<span style="font-size:24px"> `r "\U1F5FF"` : </span>
Given any point in the figure above, how can you tell its (predicted) probability approximately?<br>
```{r}
#當點落在藍線以上(右)的空間時，代表y=1的機率>0.5，預測值為y=1。
#當點落在藍線以下(左)的空間時，代表y=1的機率<0.5，預測值為y=0。
```
...
<span style="font-size:24px"> `r "\U1F5FF"` : </span>
What are the definition of `logit` & `logistic function`? What is the relationship between them?<br>
```{r}
#logit：對數勝算，將勝算（比）取log函數，其值會介於-∞到∞間。可視為x的線性組合。
#logistic function：邏輯函數，將logit（對數勝算）轉換為y=1的機率值，其值會介於0到1之間。用於求出類別變數的目標函數機率。
#當logit線性組合=0時，[y=1]的機率=0.5；logit>0時，[y=1]的機率>0.5；logit<0時，[y=1]的機率<0.5。
```
