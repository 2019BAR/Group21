...
```{r}
data=read.csv("data/mvtWeek1.csv", stringsAsFactors=F)
```
...
```{r}
#
ts[data$Arrest] %>% format('%m') %>% table %>% sort
```
...
```{r}
#
table(data$Arrest,format(ts,"%Y"))%>% prop.table(2)%>% round(3)
```

【3.4】 For what proportion of motor vehicle thefts in 2007 was an arrest made?
```{r}
#
tapply(data$Arrest, format(ts,'%Y'), mean) %>% round(3)
```
mean(?)
...
```{r}
#
table(data$LocationDescription)%>%sort%>%tail(6)
```

【4.2】 How many observations are in Top5?
```{r}
#
(top5 = names(table(data$LocationDescription) %>% sort %>% tail(6))[-4])
sum(data$LocationDescription %in% top5)
```

【4.3】 One of the locations has a much higher arrest rate than the other locations. Which is it? 
```{r}
#
TOP5 = subset(data, LocationDescription %in% top5)
